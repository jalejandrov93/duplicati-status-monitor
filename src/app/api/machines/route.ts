...
let cachedData: MachineStatus[] | null = null;
let cachedTime = 0;
const CACHE_TTL = 15000;

export async function getMachinesData(): Promise<MachineStatus[]> {
  const now = Date.now();
  ...
  const machinesWithHealth: MachineStatus[] = machines.map((machine) => {
    ...
    const hoursSinceLastBackup = (nowDate.getTime() - lastBackupTime.getTime()) / (1000 * 60 * 60);
